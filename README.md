...
**[English Version]**
This project evaluates the operational performance and SLA compliance of 49,800 shipments across four major ports. System-wide, the average turnaround time is maintained at 23.51 hours. However, deep-dive analysis reveals critical bottlenecks threatening high-margin revenue[cite: 6].

*   **Key Insight 1 (The SLA Inversion):** A significant anomaly exists where Premium clients experience a higher delayed rate (12.18%) than Standard clients (11.72%), posing a severe churn risk for high-value accounts[cite: 6].
...
**[Vietnamese Version]**
Dự án đánh giá hiệu suất vận hành và mức độ tuân thủ SLA của 49.800 chuyến hàng qua 4 cảng lớn. Thời gian xử lý trung bình toàn hệ thống là 23.51 giờ. Phân tích chuyên sâu đã chỉ ra các nút thắt đe dọa nguồn thu biên lợi nhuận cao[cite: 6].
...
*(Note: Please replace the link above with your actual GitHub Pages URL once hosted).*

---

## 2. Business Context (Bối cảnh dự án)
Dự án tập trung phân tích dữ liệu vận hành của các chuyến hàng qua cảng. Mục tiêu là phát hiện các nút thắt cổ chai trong quy trình, đánh giá hiệu suất xử lý (Turnaround Time) và đo lường mức độ tuân thủ cam kết dịch vụ (SLA Compliance) tại các cảng xuất phát khác nhau[cite: 6].

## 3. Business Questions (Câu hỏi cốt lõi)
1. **Turnaround Time:** Thời gian xử lý trung bình của lô hàng từ lúc đến (Arrival) đến lúc đi (Departure) là bao nhiêu?[cite: 6]
2. **Bottleneck Analysis:** Cảng xuất phát (Origin Port) nào đang là điểm nghẽn gây ra tỷ lệ trễ (Delayed Rate) cao nhất?[cite: 6]
3. **Volume & Cost Distribution:** Phân bổ khối lượng và chi phí vận chuyển theo từng nhóm khách hàng (Client Type) và ngành hàng diễn ra như thế nào?[cite: 6]

## 4. Data Pipeline & Processing (Nhật ký xử lý dữ liệu)
* **Quy mô:** 50,100 dòng dữ liệu ban đầu[cite: 6].
* **Xử lý (Python, SQL & DAX):**
  * Loại bỏ 100 dòng dữ liệu trùng lặp ẩn bằng quy tắc subset ID[cite: 6].
  * Impute 501 giá trị thiếu của cột `Weight_Tons` bằng Median[cite: 6].
  * Loại bỏ 201 dòng khuyết thiếu `Arrival_Time` để đảm bảo tính toàn vẹn chuỗi thời gian[cite: 6].
  * Chuyển đổi toàn bộ kiểu dữ liệu String sang Datetime chuẩn[cite: 6].
  * Đóng gói vào cơ sở dữ liệu theo mô hình Star Schema (gồm 1 Fact Table `Fact_Shipments` và các Dim Tables `Dim_Clients`, `Dim_Ports`, `Dim_Date`)[cite: 6].
...
*(Đính kèm hình ảnh Dashboard Power BI thực tế của anh tại đây)*
